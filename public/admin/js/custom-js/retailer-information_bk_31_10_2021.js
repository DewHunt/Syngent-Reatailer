jQuery(function(){
		...
		jQuery.ajax({
			...
			success:function(response) {
				...
				if(response.errors) {
					...
					if(response.errors.retailer_address){
						jQuery( '#address-error' ).html( response.errors.retailer_address[0] );
					}
	...
	jQuery(document).on("click","#editInfo",function(e){
	  ...
	  var url = "retailer.edit"+"/"+RetailId;
	  jQuery.ajax({
			url:url,
			type:"GET",
			dataType:"JSON",
			...
			success:function(response){
				...
				jQuery('.UpdateApiRetailerAddress').val(response.retailer_address);
		...
		jQuery.ajax({
			...
			success:function(response) {
				...
				if(response.errors) {
					...
					if(response.errors.retailer_address){
						jQuery( '#update-address-error' ).html( response.errors.retailer_address[0] );
					}
					if(response.errors.owner_name){
