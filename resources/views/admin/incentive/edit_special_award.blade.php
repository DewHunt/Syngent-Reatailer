...
@include('admin.incentive.menu')
    ...
    <div class="bgc-white p-20 bd">
        ...
        <div class="mT-30">
            ...
            @php $groupCatList = explode(',',$AwardInfo['group_category_id']); @endphp
                    ...
                    <select class="select2" multiple="multiple" data-placeholder="Select Category" style="width: 100%;" name="group_category_id[]" required="">
                        ...
                        @if(isset($CategoryList))
                            @foreach($CategoryList as $row)
                                <option value="{{ $row->id }}" @if(in_array($row->id,$groupCatList)) selected="selected" @endif>{{ $row->name }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
