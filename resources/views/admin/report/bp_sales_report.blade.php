...
<style type="text/css">
    ...
    */
    ...
    and (-webkit-min-device-pixel-ratio: 2) {
        ...
        .bp-select select.form-control:not([size]):not([multiple]) {
...
<div class="row">
    ...
    <div class="masonry-item col-md-9 mY-10">
        <div class="bgc-white p-20 bd new-bp-css">
            <form method="post" action="{{ url('bpDateRangesalesReport') }}">
                @csrf
                <div class="row">

                    <div class="form-group col-md-4 bp-col-xs">
                        <label>BP</label>
                        <input type="text" id="bp_search" class="form-control ui-autocomplete-input startDateMl-input" placeholder="Search By Name or Phone"/>
                        <input type="hidden" id='bp_id' name="bp_id" class="form-control">
                    </div>

                    <div class="form-group col-md-2 bp-col-sm startDateMl">
                        <label>Start Date</label>
                        <input type="text" class="form-control datepicker getStartDate" name="start_date" value="{{ (session()->get('salesSdate')) ? session()->get('salesSdate') : date('Y-m-01')  }}">
                    </div>

                    <div class="form-group col-md-2 bp-col-sm">
                        <label>End Date</label>
                        <input type="text" class="form-control datepicker getEndDate" name="end_date" value="{{ (session()->get('salesEdate')) ? session()->get('salesEdate') : date('Y-m-t')  }}">
                    </div>

                    <div class="form-group col-md-2 bp-col-sm bp-select" style="padding: 0px 5px;">
                        <label>Data Ordering</label>
                        <select class="form-control" style="width: 100%;" name="order_by" id="order_by">
                            <option value="DESC" @if(Session::get('orderBy') == 'DESC') selected="selected" @endif>Descending</option>
                            <option value="ASC" @if(Session::get('orderBy') == 'ASC') selected="selected" @endif>Ascending</option>
                        </select>
                    </div>
                ...
                <table id="dataExport" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="sorting" data-sorting_type="asc" data-column_name="total_qty" style="cursor: pointer;">BP Sales Qty</th>
                            <th class="sorting text-right" data-sorting_type="asc" data-column_name="total_sale_amount" style="cursor: pointer;">BP Sales Amount</th>
                            <th class="text-center">Order Details</th>
                        </tr>
                    </thead>
                    <tbody>
                        @include('admin.report.bp_result_data')
                    </tbody>
                </table>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
                                    ...
                                    <tr>
                                        ...
                                        <th class="totSaleQty text-bold"></th>
                                        <th class="text-right totalSalePrice text-bold"></th>
                                    </tr>
                                </table>
                            </div>
                        </div>
...
jQuery(document).on("click","#exportExcel",function(e){
    ...
    var url = "bp-sales-export?"+searchData;
    ...
    jQuery.ajax({
        ...
        success:function(response) {
            ...
            if(response) {
                ...
                jQuery('#itemList').html(response.itemList);
            }
        }
    });
});
