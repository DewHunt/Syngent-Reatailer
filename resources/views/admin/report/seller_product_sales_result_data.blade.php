@if(isset($productSalesReport) && !$productSalesReport->isEmpty())
    ...
    <tr>
        ...
        <button type="button" data-id="{{ $row->product_model }}{{'~'}}{{ ($row->bp_id > 0) ? 'bp_id':'retailer_id' }}{{'~'}}{{ ($row->bp_id > 0) ? $row->bp_id:$row->retailer_id }}" id="SellerProductSalesDetails" class="btn cur-p btn-info btn-xs eyeViewbtn" data-toggle="modal" data-target="#viewProductSalesDetailsModal"><i class="fa fa-eye" aria-hidden="true"></i></button>
        </td>
