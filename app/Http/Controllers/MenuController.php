...
class MenuController extends Controller
{
    public function index(Request $request) {
        ...
        $menuList = DB::select("
            ...
            where `tab1`.`is_full_off` = 0
            union
            (select `menus`.*, `parent_menu` as `parentName` from `menus` where `parent_menu` is null AND `is_full_off` = 0)
            order by `id` asc;
        ");
    ...
    public function edit($menuId) {
        ...
        if (!empty($menuInfo)) {
            $html = view('admin.menu.edit_form')->with(compact('menuList','menuInfo'))->render();
    ...
    public function update(Request $request) {
        ...
        if ($checkMenuLink && $request->menuLink != null) {
            $status = false;
            return response()->json('error');
        } else {
    ...
    public function deleteMenu(Request $request) {
        // dd($request->all());
        $status = Menu::where('id',$request->id)->delete();
    ...
    public function changeStatus($id)  {
        if (isset($id) && $id > 0) {
            ...
            $UpdateMenuStatus = menu::where('id',$id)->update([
