...
class JwtAuthController extends Controller
{
        ...
        if (isset($CheckUserList) && !empty($CheckUserList)) {
            ...
            if ($CheckUserList->bp_id > 0 && $CheckUserList->brand_promoter_phone == $login_phone_number) {
                ...
                ApiLoginActivity::create([
                    "type" => "BP Successfully Logged In",
            ...
            } else if ($CheckUserList->retailer_id > 0 && $CheckUserList->retailer_phone == $login_phone_number) {
                $dataArray = [
                ...
                ApiLoginActivity::create([
                    "type" => "Retailer Successfully Logged In",
                    "user_agent" => $request->server('HTTP_USER_AGENT'),
                    "user_id" => $CheckUserList->id,
                    "ip_address" => $request->ip(),
                    "created_at"=>Carbon::now(),
                ...
                ApiLoginActivity::create([
                    "type" => "Employee Successfully Logged In",
                    "user_agent" => $request->server('HTTP_USER_AGENT'),
                    "user_id" => $CheckUserList->id,
                    "ip_address" => $request->ip(),
                    "created_at"=>Carbon::now(),
    ...
    */
    protected function generateToken($token){
        $retailerId = auth($this->guard)->user()->retailer_id;
        $retailerInfo = Retailer::find($retailerId);
        auth($this->guard)->user()->retailerInfo = $retailerInfo;
