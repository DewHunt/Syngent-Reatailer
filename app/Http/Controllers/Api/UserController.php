...
class UserController extends Controller
{
    public function index(Request $request)
    {
        ...
        //$GetUser = DB::table('users')->orderBy('name','ASC')->paginate(500);
        $GetUser  = DB::table('users')->where('employee_id','>',0)->orderBy('name','ASC')->get();
        if($request->ajax()) {
    ...
    public function GetUserList(Request $request)
    {
    	...
    	$userLists  = DB::table('view_check_login_user')
        ->where('employee_id','>',0)
        ->orderBy('name','ASC')
        ->get();
    	//User::paginate(10);
